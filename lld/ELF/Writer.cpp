...
SmallVector<PhdrEntry *, 0> Writer<ELFT>::createPhdrs(Partition &part) {
  ...
  for (OutputSection *sec : outputSections) {
    ...
    // FIXME: Added specifics for nanoMIPS custom linker script, as parsing
    // OVERLAY in linker script is different for this architecture. New load
    // program header is needed so OVERLAY sections can have same virtual
    // addresses but different load addresses
    ...
    if (!(load && newFlags == flags && sec != relroEnd &&
        ...
        (config->nanoMipsCustomLinkerScriptType &&
