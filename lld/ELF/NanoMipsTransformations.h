...
struct NanoMipsSymbolAnchor {
  Defined *d;
  // if a symbol has size, it will have an anchor for both its start and
  // end, if end is true that means that that is the place where symbol
  // ends (usually a function symbol)
  bool end;
...
private:
  MutableArrayRef<NanoMipsRelocProperty *> getRelocTableMut() {
    return MutableArrayRef(table, RelocPropertyTableSize);
  }
  ArrayRef<NanoMipsRelocProperty *> getRelocTable() const {
    return ArrayRef(table, RelocPropertyTableSize);
  }
...
public:
  ...
  ArrayRef<NanoMipsInsTemplate> getInsns() const {
    return ArrayRef(insns, insCount);
  }
