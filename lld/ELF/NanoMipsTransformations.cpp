...
static Defined *addSyntheticLinkerSymbol(const Twine &name, uint64_t val,
...
static uint64_t readInsn(ArrayRef<uint8_t> data, uint64_t off,
                         uint32_t insnSize) {
...
template <endianness E>
static void writeInsn(uint64_t insn, ArrayRef<uint8_t> data, uint64_t off,
                      uint32_t insnSize) {
