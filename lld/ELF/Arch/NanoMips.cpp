...
void writePcRel16(uint8_t *loc, uint64_t val, uint8_t bitsSize) {
  ...
  val = (val & ~1ULL) | ((val >> bitsSize) & 1ULL);
...
void NanoMips<ELFT>::relocateAlloc(InputSectionBase &sec, uint8_t *buf) const {
  ...
  for (auto [i, rel] : llvm::enumerate(sec.relocs())) {
    ...
    // If prevRelocOnlyCalculating is correct previous relocation was part of
    // a composite relocation (see comment below), use its calculated value as
    // this relocation's addend
    // TODO: See if maybe addend of the current relocation
    // should be added to the previous reloc's value instead of
    // just ignoring it
    int64_t addend = prevRelocOnlyCalculating ? valFromBefore : rel.addend;
    val = SignExtend64(sec.getRelocTargetVA(sec.file, rel.type, addend,
